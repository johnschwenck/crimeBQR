...
We propose a spatial adaptation of the \textit{Bayesian Multiple Quantile Regression for Linear Models Using a Score Likelihood} by  [Wu \& Narisetty (2020)](https://projecteuclid.org/euclid.ba/1596160823) and use the "Ada-IS" importance sampling algorithm for computing the posterior summaries.
...
Oftentimes, the commonly used conditional mean regression is sensitive to extreme events / outliers and fails to characterize the entire distribution. Quantile regression (QR) can be utilized to investigate specific parts of a distribution.
...
- Previous attempts have made use of the Asymmetric Laplace Distribution, but without valid frequentist inference. The paper successfully incorporated a \textit{working} likelihood to incorporate the check loss function
...
However, in order to properly account for the spatial dependencies in the data, we reformulate the weight matrix $\textbf{W}$ with our own non-parametric approach using a spatial spline-based weight matrix
